        ...
        <div class="row">
            <div class="col-lg-4 col-md-6">
                ...
                <select name="station" class="form-control" onchange="this.form.submit()">
                    ...
                    {{-- @php $stations = $stations ?? config('stations.data'); @endphp --}}
                    @foreach ($stations as $key => $station)
                        <option value="{{ $station['code'] }}" {{ isset($selected_station) && $selected_station  == $station['code'] ? 'selected' : '' }}>{{ $station['namen']['lang'] }}</option>
                    @endforeach
                </select>
                @error('station')
